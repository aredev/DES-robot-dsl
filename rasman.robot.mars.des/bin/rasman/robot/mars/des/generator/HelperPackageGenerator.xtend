...
class HelperPackageGenerator {
		
	def static generateHelpers(String file){
		val RGB = '''
		package helpers;
		...
		public final class RGBColorWrapper {
			 ...
			 */
			static float[] green = { 0.10f, 0.19f, 0.08f };
			/**
			...
			/**
			*	Gray RGB color measurements. Color ID 9
			*/
			static float[] gray = {  0.009f, 0.009f, 0.009f};
			...
			public static int determineColor(float[] colorMeasurement){
				...
				else if (equalMeasurements(black, colorMeasurement, 0.009f)){
					return Color.BLACK; // Color ID 7
				}else if (equalMeasurements(gray, colorMeasurement, 0.005f)){
					return Color.GRAY; // Color ID 9
				}
				else{
					return Color.NONE;
				}
		...
		val IgnoreBehaviors = '''
			...
			public class IgnoreBehaviors {
				
				public static boolean withOnDistanceBehavior = true;		
				public static int currentTaskId = 0;
				public static int totalNrOfTasks = 999;
			}
		'''
		
		val IncrementidTimerTask = '''
		package helpers;
